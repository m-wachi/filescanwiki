...
LOG_STD = "filescanwiki05.log"
LOG_ERR = "filescanwiki05_err.log"
...
def getNextSibling(curDir, curFile):
    ...
    lstDirEnt = list(map(pathNormalize, sorted(os.listdir(curDir))))
...
class TracDb:
    ...
    def register2TracDb(self, pageName, pageData, filepath):
        ...
        if recCnt == 0:
            ...
            sql += "  (NAME,VERSION,TIME,AUTHOR,TEXT,COMMENT,READONLY) \n"
            sql += "values(%s, 1, %s, 'filescanner', %s, '', 0) "
...
def logstd(txt):
    ...
    glbLogStdF.write(txt)
    glbLogStdF.write('\n')
    glbLogStdF.flush()

def logerr(txt):
    #glbLogErrF.write(txt.encode('cp932'))
    glbLogErrF.write(txt)
    glbLogErrF.write('\n')
    glbLogErrF.flush()
